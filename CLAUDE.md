...
# 考研戰術協議：線性代數反射參照表 (Reflex Table Protocol)

## 1. 核心目標 (Core Objective)
本協議旨在協助使用者（資工所考生）將原子筆記轉化為「看到...想到...」的高速檢索索引。目標是讓使用者在面對線性代數考題時，能快速擷取多個題目特徵，並根據這些反射出的原子動作，直接在大腦中拼湊出解題路徑，達到 $O(1)$ 的反射速度。

## 2. 語意提取邏輯 (Semantic Extraction Logic)
當收到「更新反射表」的指令時，Claude 必須：
- **深度分析內文**：讀取原子筆記中的定理、性質、推論與條件結論表。
- **邏輯對稱化**：將筆記中的「若 A 則 B」或「A iff B」自動轉化為「看到 A (題目特徵) -> 想到 B (工具/性質)」的映射關係。
- **Multimap 構建**：保留一對多的映射關係。一個特徵可能指向多個不同的性質，必須完整呈現，不可合併或刪除，以供解題時拼湊使用。

## 3. 輸出限制與格式 (Output Constraints)
- **嚴格去冗餘**：禁止輸出證明過程、詳細計算步驟或多餘的解釋。
- **視覺特徵導向**：輸出的「看到 (Signal)」必須精煉為題目中會出現的視覺關鍵字（如：$Ax=0$ 有非零解、矩陣對稱、$m < n$ 等）。
- **表格格式**：
  | 看到 (Signal / 題目特徵) | 想到 (Mapping / 原子反射動作) | 來源筆記 |
  | :--- | :--- | :--- |

## 4. 戰術指導原則 (Tactical Principles)
- **讀書首位**：筆記保持使用者原本的學術結構，AI 負責自動化提取，不強制使用者修改筆記格式。
- **模組化拼湊**：反射動作必須保持原子性，讓使用者能組合「特徵 A 的反射」+「特徵 B 的反射」來對付複合型考題。
