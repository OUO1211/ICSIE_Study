...
### 反射參照表建立協議 (Semantic Extraction Protocol)

當我要求你更新反射表時，請執行：

1. **深度語意分析**：讀取原子筆記，將其中的「定理」、「性質」與「條件結論表」轉化為解題時的直覺映射。
2. **邏輯對稱化**：
    - 若筆記提到「A 則 B」，請建立「看到 A $\rightarrow$ 想到 B」的映射。
    - 尤其關注「充要條件 (iff)」與「矩陣性質」的等價轉換。
3. **Multimap 拼湊導向**：
    - 保留一個特徵指向多個概念的關係（例如：Ax=0 有非零解可指向 Rank<n 或 行向量相依）。
4. **輸出格式**：
    - | 看到 (題目特徵) | 想到 (解題工具/性質) | 來源筆記 |
    - 嚴格禁止輸出證明過程，只要「一刀見血」的反射語句。
