...
這是一份為你量身打造、整合了所有戰術細節的完整 `CLAUDE.md` 內容。它將你的「讀書首位」原則、原子筆記習慣，以及追求 $O(1)$ 反射的資工所考研目標全部自動化。

你可以直接複製以下內容並覆蓋你的 `CLAUDE.md`：

---

# 頂大資工考研：特徵反射與拼湊協議 (Feature-Based Reflex Protocol)

## 1. 核心目標 (Core Objective)

本協議旨在將高品質的原子筆記轉化為「看到 (Signal) -> 想到 (Mapping)」的極簡索引。其終極目標是讓使用者在面對線性代數等考研題目時，能實現 $O(1)$ 的特徵擷取速度，並根據多個原子特徵的組合（拼湊），直接在腦中推導出最終答案或解題路徑。

## 2. 語意壓縮邏輯 (Semantic Extraction Logic)

- **超越公式表層**：不只是提取公式，必須提取公式背後的「數學本質」、「適用邊界」或「隱含約束」。
    
- **Multimap 結構**：若一個特徵對應多個性質，必須全部列出，不可合併。這對處理「多選觀念題」至關重要。
    
- **原子化 Mapping**：輸出的動作必須具備獨立性，以便使用者將不同的 Mapping 進行「邏輯交集」來對付複合型考題。
    
- **讀書首位原則**：直接從使用者的學術筆記內文提取邏輯，不強制要求特定標籤（如 YAML），以維持學習流暢度。
...
- **範例 A（關於齊次方程）**
    ...
    - **想到**：$\operatorname{rank}(A) < n$；$A$ 為奇異矩陣 (Singular)；$\det(A) = 0$
        
    - **抽象意義**：矩陣發生「維度損耗」，存在資訊被壓縮至零空間。
        
- **範例 B（關於矩陣乘法）**
    
    - **看到**：$AB = AC$ 但 $A$ 狀態未知
        
    - **想到**：不可直接消去 $A$；暗示 $A(B-C) = O$ 存在零因子
        
    - **抽象意義**：非交換代數中，等號不代表可逆，需檢查 **Full Rank**。
        
- **範例 C（關於轉置結構）**
    
    - **看到**：題目出現 $A^T A$
        
    - **想到**：恆為對稱矩陣；其 $\operatorname{tr}(A^T A)$ 為元素平方和；具備半正定性
        
    - **抽象意義**：將一般矩陣轉化為具備「優良幾何性質」的對稱結構。
...
- **範例 D（關於解結構）**
    ...
    - **抽象意義**：線性算子的平移空間性質；解的唯一性取決於 $\ker(A)$ 是否為零向量。
        

## 5. 執行指南 (Instructional Guidelines)

- **嚴格禁令**：禁止輸出任何證明過程、詳細計算步驟或冗長解釋。
    
- **自動化指令**：當使用者要求「更新反射表」時，應掃描指定筆記，根據上述協議產出極簡、可組合的反射項目。
