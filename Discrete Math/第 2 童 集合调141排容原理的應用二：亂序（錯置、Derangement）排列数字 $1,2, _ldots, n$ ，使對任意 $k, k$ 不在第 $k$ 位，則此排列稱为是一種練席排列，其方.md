...
排列数字 $1,2, \ldots, n$ ，使對任意 $k, k$ 不在第 $k$ 位，則此排列稱为是一種練席排列，其方法数記為 $D_n$ ，則 $D_n=n!\times\left[1-\frac{1}{1!}+\frac{1}{2!}-\frac{1}{3!}+\ldots+(-1)^n \frac{1}{n!}\right]=n!\times\left(\sum_{k=0}^n(-1)^k \frac{1}{k!}\right)$
...
（2）亂序數推廣：$n$ 相異數的排列法中，
（1）佮 $r$ 個數不在正確位置上的方法數 ：$\binom{n}{r} D_r$ 。
（2）$r$ 個數以上不在正確位置上的方法數 ：$\sum_{k=r+1}^n\binom{n}{k} D_k$ 。
（3）葻序數的遞迴求算技巧：$\left\{\begin{array}{l}D_n=(n-1)\left(D_{n-1}+D_{n-2}\right), \forall n \geq 3, \\ D_1=0, D_2=1\end{array}\right.$
（148頁有證明）。