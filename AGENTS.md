...
> 注意：以下章節資料夾實際路徑皆在 `Linear Algebra (上)/第N章/`（阿拉伯數字，非中文數字），非早期記錄誤寫的 vault 根目錄 `/第一章/`。

### 第一章：矩陣（`Linear Algebra (上)/第1章/`）
**MOC 檔案**：`矩陣(MOC).md`
...
### 第二章：線性方程組與行列式（`Linear Algebra (上)/第2章/`）
**MOC 檔案**：`線性方程組與行列式(MOC).md`
...
### 第三章：向量空間（`Linear Algebra (上)/第3章/`）
**MOC 檔案**：`向量空間(MOC).md`

已建立的原子筆記（共 17 個）：向量空間定義、常見向量空間、子空間、子空間的和、四大基本子空間、線性組合與生成空間、線性獨立與相依、Wronskian、基底與維度、基底三大定理、基底維度性質、四大子空間的基底、和空間維度公式、矩陣的秩、秩與線性系統、座標與基底變換、直和。

原始頁碼範圍約 254–433（含 3-7 進階試題練習頁 429–433，內容已按主題併入相關原子筆記，未單獨立檔）。

---

### 第四章：線性映射（`Linear Algebra (上)/第4章/`）
**MOC 檔案**：`線性映射(MOC).md`
...
| 節次 | 檔案 |
|------|------|
| 4-1 線性映射 | `線性映射定義與判別.md`、`線性映射的存在性與唯一性.md`、`特殊線性映射-旋轉與鏡射.md`、`特殊線性映射-投影與斜投影.md`、`冪等算子與冪等矩陣.md` |
| 4-2 矩陣表示 | `線性映射的矩陣表示.md`、`座標變換公式.md`、`換底變換公式.md`、`線性算子的行列式與跡數.md`、`線性映射的合成.md`、`可逆的線性映射.md` |
| 4-3 核空間與值域 | `核空間與值域定義.md`、`求核空間與值域方法.md`、`T平方的核空間與值域.md` |
| 4-4 一對一與映成 | `一對一與映成的判別.md`、`線性映射與保相依保獨立保生成.md`、`向量空間的同構.md` |
| 4-5 不變子空間 | `不變子空間.md` |

原始頁碼範圍 444–576（含 4-6 進階試題練習頁 568–576，因應下方「原子筆記不放考題」規則，未併入筆記，內容留在原始 `Obsidian_Notes(success version)/線代(上)_Page_XXX.md` 供之後另行練習之用）。
...
`Linear Algebra Note-Taking Skill` 原文列出 `## 經典考題` 為標準區塊，但使用者已明確要求**所有科目**的原子筆記一律只放觀念（定義／性質／證明／核心模型），不放經典考題或成套例題，理由是考題會稀釋「一檔一觀念」的原子性。第四章筆記已依此規則產出；第三章（及更早章節）少數帶有「經典考題」小節的舊筆記尚未回頭清理，之後若有機會編輯到，應一併移除。
...
### 第2章：陣列與結構（`/Data Structure/第二章/`，尚未建立 MOC，依使用者指示暫緩）

已建立的原子筆記（共 7 個）：

| 主題 | 檔案 |
|---|---|
| 串列與陣列基礎 | `串列與陣列基礎List-Array.md` |
| 位址計算 | `二維陣列位址計算RowColumnMajor.md`、`多維陣列位址計算N維陣列.md` |
| 多項式表示法 | `稀疏多項式的陣列表示法與相加.md` |
| 陣列運算 | `矩陣轉置TransposeMatrix.md` |
| 特殊矩陣壓縮儲存 | `三角矩陣與對稱矩陣壓縮儲存.md`、`頻帶矩陣BandMatrix.md` |

原始檔案涵蓋頁碼 68–82（本章原始頁碼範圍實際只到 82，非原先估計的 92；84–92 無對應檔案）。內容全為文字/公式/表格，無圖形類內容，未受 pdf_imgs 錯誤來源問題影響。
