...
蠋零（nilpotent）與指標（index）
（1）對缐性算子 $T$ ，若存在正整数 $k$ 使得 $T^k=O$ ，則稱 $T$ 為一賽零算子，並稱使 $T^k=O$ 成立的最小正整数 $k$ 為 $T$ 的指標（index）．
（2）對矩陣 $A$ ，若存在正整数 $k$ 使得 $A^k=O$ ，則稱 $A$ 為一暴零矩陣，並稱使 $A^k=O$ 成立的最小正整数 $k$ 為 $A$ 的指標（index）．
