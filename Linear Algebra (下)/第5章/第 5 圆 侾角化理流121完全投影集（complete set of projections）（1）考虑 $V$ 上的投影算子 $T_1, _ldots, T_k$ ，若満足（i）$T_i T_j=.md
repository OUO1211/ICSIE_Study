...
光滥分解（spectral decomposition）
放 $T \in L(V, V)$ ，且 $T$ 有相異特徵根 $\lambda_1, \ldots, \lambda_2$ ，
若 $T$ 可到角化，則
存在一完金投影集 $\left\{T_1, \ldots, T_k\right\}$ 使 $T=\lambda_1 T_1+\ldots+\lambda_k T_k$ 。
Note
（1）此稱為 $T$ 的光譜分解．
（2）影方陣 $A$ 亦有類似結論。
（3）做光梆分解的方法：
設矩娕 $A$ 可對角化且有相異特徵根：$\lambda_1, \ldots, \lambda_k$ ，設各重根數為 $m_i$ ，
